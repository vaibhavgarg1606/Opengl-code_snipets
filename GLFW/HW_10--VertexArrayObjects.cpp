...
VertexArrayObjects is used in both core(modern version) and compat (compatibility or older 0version)

but in compat there is default vao created and when we enable VertexArrayAttrib we use 0 to use that default vao
but in core we need to create vao ourself and by creating multiple vao we can just bind that vao in while loop to display the object whose data is stored in VertexArrayAtttrib
it can be created similarly to buffer;
...
int main(void)
{
    ...
    /* ----------- Unbound everything ----------- */
            ...
            GLCall(glBindBuffer(GL_ARRAY_BUFFER, 0));           //buffer
            GLCall(glBindVertexArray(0));                       // Vao
            GLCall(glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, 0));   // inbex buffer
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        /* ------------- Bind Back everything ------------- */
        GLCall(glUseProgram(shader));
        GLCall(glUniform4f(location, r, 0.2f, 0.5f, 1.0f));
            
        /* ----------- instead of binding all this just bind Vao ----------- */
            //GLCall(glBindBuffer(GL_ARRAY_BUFFER, buffer));
            //GLCall(glEnableVertexAttribArray(0));
            //GLCall(glVertexAttribPointer(0, 2, GL_FLOAT, GL_FALSE, sizeof(float) * 2, 0));
        GLCall(glBindVertexArray(vao));
