...
we get error with the help of a function "glGetError" it return any arbitrary error one at a time
so we have to use while loop to get all the error

so we use "GLClearError" to clean all the pending error and the call "GLLogError" to log error on console
where GLLogError get the error with help of glGetError and display error on screen with function name, file path and line

And we use Macros so that we dont need to use call 2 fucntion seperately 

here ASSERT Stop execution of program with the help of visual Studio compiler macro __debugBreak() 

and "GLCall" takes the function to be called as parameter and first calls GLClearError then call respective function to be called and then call GLLogError and pass function name file path and line as argument

Here
---> \ is used to be able to write macro on next line
---> # is used to get function name as string
---> __FILE__ is used to get file path
---> __LINE__ is used to get line where fucntion is executed
...
/* 
# before a fucntion pass it as a string and __FILE__and __LINE__ are macros to get file loc and line of fucntion
...
int main(void)
{
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        GLCall(glDrawElements(GL_TRIANGLES, 6, GL_UNSIGNED_INT, nullptr));   // 6 is no of indices and in 4th option we have to put address of ibo but we already bind it 
